...
- 会費: 「会費」→「会費」で対象年度の年会費と未納・完納の状態を確認する。役員による集金を希望するときは「集金」にチェックする。Stripeが利用可能ならオンラインで支払える。入金後は同じ画面から領収書を開ける。会費状態は同じ世帯で共有される。
...
- 基本機能: 「基本情報」「会員管理」「会費管理」「システム利用料」「役員管理」「Stripe連携」がある。基本情報では町内会・自治会名、決算月、世帯規模、郵便番号、代表者表示を管理する。会員管理では名簿、LINE連携、家族アカウント、退会状態を管理する。会費管理では会費設定、年間請求額、納入額、集金と会員の集金希望を管理する。システム利用料では月額世帯単価、無料プッシュ枠、超過配信数、請求額を確認する。役員管理では招待、再送、退任、復活を行う。Stripe連携ではConnect登録と決済受付状態を確認する。
...
- 総会会計: 会計科目登録、予算入力、会計処理、総会資料作成がある。会計年度を確認して収入・支出科目、予算額、決算実績を管理し、必要な帳票を出力する。
