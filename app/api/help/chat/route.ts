...
- 基本機能: 「基本情報」「会員管理」「会費管理」「システム利用料」「役員管理」「Stripe連携」がある。基本情報では団体名、決算月、世帯規模、郵便番号、代表者表示を管理する。会員管理では名簿、LINE連携、家族アカウント、退会状態を管理する。会費管理では会費設定、年間請求額、納入額、手集金を管理する。システム利用料では月額世帯単価、無料プッシュ枠、超過配信数、請求額を確認する。役員管理では招待、再送、退任、復活を行う。Stripe連携ではConnect登録と決済受付状態を確認する。
