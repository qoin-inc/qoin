...
export async function GET(req: Request) {
  try {
    ...
    const { data, error } = await createWebhookSupabaseClient().from("system_usage_bank_account").select("bank_name,bank_code,bank_branch_name,bank_branch_code,bank_account_type,bank_account_number,bank_account_holder,issuer").eq("id", 1).maybeSingle();
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const account = { ...validateBankAccount(input), issuer: validateInvoiceIssuer(input.issuer) };
