    ...
    <ManualAccessGate>
      <OnboardingGuide
        ...
        steps={[
          ...
          {
            title: "el-townで町内会・自治会情報を入力する",
            text: "管理トップの「基本機能」から「Stripe連携」を開きます。町内会・自治会名は基本情報から同期されます。Stripe連絡先メール、問い合わせ電話番号、Webサイト、サービス内容を確認・入力してください。",
            points: [
              "サービス内容には、町内会費・自治会費をオンラインで受け取る目的を具体的に記載します。",
              "Webサイトは町内会・自治会や活動内容が確認できる公開ページを入力します。",
              "登録済み内容がある場合は読み込み完了を待ち、空欄で上書きしないことを確認します。",
            ],
            caution: "町内会・自治会名を変更する必要がある場合は、先に「基本情報」で正式名称を修正してからStripe連携へ戻ってください。",
            visual: <StripeDesktopPreview focus="registration" caption="現在のPC画面：右側の「本番Stripe登録を開始」で町内会・自治会情報を入力します" />,
          },
          {
            title: "3つの準備確認後に本番登録を開始する",
            text: "「規約・登記上の組織区分」「代表者の本人確認書類」「町内会・自治会が管理する入金先口座」の3項目を実際に準備できた場合だけチェックします。「入力内容を確認して本番Stripe登録を開始」を押すと、別画面でStripeの登録が始まります。",
            points: [
          ...
          },
          {
            ...
            text: "Stripe連携が有効になったら「基本機能」→「会費管理」を開き、「Stripeカード決済」を選択して「この町内会・自治会の設定を保存」を押します。その後、会計年度、請求額、対象者を確認し「Stripe請求に設定」を押します。",
            points: [
              "会費名称、標準会費額、会員向け支払い案内も同時に確認します。会計年度は基本情報の決算月から自動計算されます。",
