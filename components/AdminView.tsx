...
const getFamilyWithdrawnSlots = (member: any) => ([1, 2] as const).filter((slot) => member[`family_withdrawal_status_${slot}`] === "withdrawn");
...
const getMemberStatusLabel = (member: any) => {
  ...
  const familyWithdrawn = getFamilyWithdrawnSlots(member);
  ...
  if (familyWithdrawn.length) return `家族${familyWithdrawn.join("・")}退会済み`;
  return "利用中";
};
...
export default function AdminView({ townId, townName, isRepresentative = false, isSystemAdmin = false }: AdminViewProps) {
  ...
  useEffect(() => {
    let active = true;
    setSystemBankAccount(null);
    setSystemBankAccountError("");
  ...
  const startFacilityEdit = (facility: any) => {
    ...
    window.requestAnimationFrame(() => document.getElementById("admin-live-facility-panel")?.scrollIntoView({ behavior: "smooth", block: "start" }));
  };
  ...
  const handleMemberRestore = async (member: any) => {
    const fullName = getMemberFullName(member);
    if (typeof window !== "undefined" && !window.confirm(`${fullName}様の退会状態を解除します。LINE連携は自動では戻らないため、本人と家族の再連携が必要です。よろしいですか？`)) return;

    const reply = `${fullName}様の退会状態を解除しました。el-townを利用するには、LINEから会員名簿との照合をもう一度行ってください。`;
    setMemberBusy(true);
    setMemberMessage("");
    setMemberReply("");
    try {
      const payload = { withdrawal_status: "active", withdrawal_reply_message: reply };
      const saved = await updateRowWithFallback("resident_rosters", member.id, payload, "退会状態の解除に失敗しました。");
      const updated = { ...member, ...payload, ...saved };
      setBasicData((current) => ({
        ...current,
      ...
      setMemberReply(reply);
      setMemberMessage("退会状態を解除しました。本人と家族はLINE連携をやり直してください。");
    } catch (error: any) {
      setMemberMessage(error?.message || "退会状態の解除に失敗しました。");
    } finally {
      setMemberBusy(false);
    }
  };

  const handleFamilyRestore = async (member: any, slot: 1 | 2) => {
    const familyName = member[`family_name_${slot}`] || `家族${slot}`;
    if (typeof window !== "undefined" && !window.confirm(`${familyName}様の退会状態を解除します。LINE連携は自動では戻らないため、再連携が必要です。よろしいですか？`)) return;

    setMemberBusy(true);
    setMemberMessage("");
    try {
      const payload = { [`family_withdrawal_status_${slot}`]: "active" };
      const saved = await updateRowWithFallback("resident_rosters", member.id, payload, "家族の退会状態の解除に失敗しました。");
      const updated = { ...member, ...payload, ...saved };
      setBasicData((current) => ({
        ...current,
        members: current.members.map((item) => item.id === member.id ? updated : item),
      }));
      setMemberMessage(`${familyName}様の退会状態を解除しました。LINE連携をやり直してください。`);
    } catch (error: any) {
      setMemberMessage(error?.message || "家族の退会状態の解除に失敗しました。");
    } finally {
      setMemberBusy(false);
    }
  };
...
<body>
  ...
  const integratedFacilityCards = liveFacilityData.facilities.filter((facility) => (
        ...
        <div className="admin-publish-heading">
          ...
          <div className="admin-heading-actions">
            ...
            <button type="submit" disabled={publishBusy}>
              ...
              <span>{publishBusy ? "保存中" : editingPublishId ? "発信を更新" : "発信"}</span>
            </button>
          </div>
        </div>
          ...
          <section className="admin-basic-card admin-member-list">
            ...
            <div className="admin-member-table">
              ...
              {(filteredMembers.length ? filteredMembers : [{ id: "empty", full_name: basicData.members.length ? "検索条件に一致する会員はいません" : "会員名簿は未取得です", status: "未設定" }]).map((member, index) => {
                ...
                const familyWithdrawnSlots = getFamilyWithdrawnSlots(member);
      ...
      <div className="admin-stripe-screen">
        <section className="admin-basic-card admin-stripe-status">
          <div className="admin-basic-card-heading">
            <div>
              <h3>Stripe連携</h3>
              <p><strong>el-townではインターネット上で安全かつ簡単に決済システムを導入できる、世界標準のオンライン決済プラットフォームであるStripeを標準採用しています。Stripeは世界で数百万社以上の企業に利用されております。</strong></p>
              <p><strong>町内会・自治会がStripeに登録し契約することにより、クレジットカード決済や口座振込、PayPay(オプション）を利用可能になります。</strong></p>
            </div>
            <span className={stripeReadyForFeeBilling ? "admin-stripe-badge ready" : rawStripeAccountId ? "admin-stripe-badge pending" : "admin-stripe-badge"}>
              {stripeRegistrationStatusLabel}
            </span>
            ...
            <section className="admin-integrated-reservations" aria-label="施設予約管理">
              ...
              <div className="admin-facility-groups">
                {integratedFacilityCards.map((facility) => {
                  const reservations = filteredIntegratedFacilityReservations.filter((reservation) => String(facilityReservationFacilityId(reservation)) === String(facility.id));
                  return (
                    <article key={facility.id} className="admin-facility-group">
                      <header>
                        <span>
                        ...
                        <div>
                          <button type="button" onClick={() => startFacilityEdit(facility)} disabled={liveFacilityBusy}>修正</button>
                          <button type="button" className="delete" onClick={() => handleFacilityDelete(facility)} disabled={liveFacilityBusy}>削除</button>
                        </div>
                      </header>
                      <div className="admin-facility-group-reservations">
                        {reservations.map((reservation) => (
                          <div key={reservation.id} className={`admin-live-reservation ${reservation.status || "pending"}`}>
                            <span>
                              <strong>{toDisplayDate(reservation.reservation_date)} {reservation.start_time || ""}{reservation.end_time ? `-${reservation.end_time}` : ""}</strong>
                        ...
                        {reservations.length === 0 && <div className="el-empty">指定した期間の予約はありません。</div>}
                      </div>
                    </article>
                  );
                })}
                {integratedFacilityCards.length === 0 && <div className="el-empty">登録施設はありません。</div>}
              </div>
            </section>
          )}
          <div className="admin-work-list">
