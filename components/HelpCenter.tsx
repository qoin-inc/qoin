...
const memberCategories: HelpCategory[] = [
  ...
  },
  {
    ...
    operations: [
      { label: "会費", description: "年会費・集金・Stripe支払い", answer: "「会費」→「会費」を押すと、対象年度の年会費と未納・完納の状態を確認できます。役員による集金を希望する場合は「集金」にチェックします。Stripeが利用可能な団体ではオンラインで支払えます。会費の状態は同じ世帯で共有されます。" },
    ],
  },
  {
...
const adminCategories: HelpCategory[] = [
  {
    id: "basic", label: "基本機能", description: "町内会・自治会・会員・会費・役員・決済", icon: "fa-layer-group",
    operations: [
      { label: "基本情報", description: "町内会・自治会情報を確認・変更", answer: "管理トップで「基本機能」→「基本情報」を押します。町内会・自治会名、決算月、世帯規模、郵便番号、代表者表示などを確認・変更し、保存してください。" },
      { label: "会員管理", description: "名簿・連携・退会状態", answer: "管理トップで「基本機能」→「会員管理」を押します。連携済み会員、未連携名簿、家族アカウント、退会状態を確認し、必要な会員情報を登録・修正してください。" },
      { label: "会費管理", description: "請求・入金・集金", answer: "管理トップで「基本機能」→「会費管理」を押します。会費設定、年間請求額、納入額、支払い方法を確認できます。会費一覧の「集金希望のみ」で未納の希望世帯を絞り込めます。役員が現金を受け取った場合は、対象会員の集金額を入力して保存してください。" },
  ...
  },
  {
    ...
    operations: [
      { label: "会計科目登録", description: "収入・支出科目を設定", answer: "管理トップで「総会会計」→「会計科目登録」を押します。収入・支出の科目や補助科目を追加・編集し、表示順を設定してください。" },
      { label: "予算入力", description: "年度予算を入力", answer: "管理トップで「総会会計」→「予算入力」を押します。会計年度を選び、各科目の予算額と備考を入力して保存してください。" },
      { label: "会計処理", description: "収入・支出実績を入力", answer: "管理トップで「総会会計」→「会計処理」を押します。会計年度と対象期間を確認し、科目、日付、金額、内容を入力して保存してください。" },
      { label: "総会資料作成", description: "会計帳票を出力", answer: "管理トップで「総会会計」→「総会資料作成」を押し、年度と集計内容を確認します。「予算CSV」「決算CSV」「予算PDF/印刷」「決算PDF/印刷」から必要な形式を選んでください。" },
    ],
  },
];
...
function answerHelpQuestion(audience: HelpAudience, question: string) {
  ...
  if (audience === "member") {
    ...
    if (/会費|支払|決済|領収|請求|入金/.test(normalized)) return "画面下の「会費」を押すと、年会費と未納・完納の状態を確認できます。集金を希望する場合は「集金」にチェックしてください。Stripeが利用可能ならオンラインで支払えます。";
  ...
  if (/総会.*(会計|予算|決算|科目)|予算|決算|科目|csv|印刷/.test(normalized)) return "管理トップで「総会会計」を押します。会計科目登録、予算入力、会計処理、総会資料作成から必要な操作を選んでください。";
  ...
  if (/会費|支払|入金|集金/.test(normalized)) return "管理トップで「基本機能」→「会費管理」を押すと、会費設定、請求額、入金状況、会員の集金希望を管理できます。";
  ...
  if (/基本情報|町内会・自治会情報|決算月|郵便番号/.test(normalized)) return "管理トップで「基本機能」→「基本情報」を押し、町内会・自治会名、決算月、世帯規模、郵便番号、代表者表示を確認・変更してください。";
