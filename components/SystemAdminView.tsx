...
export default function SystemAdminView() {
  ...
  const loadBillingStatus = async () => {
    ...
    const data = await response.json().catch(() => ({}));
    setBillingEnabled(Boolean(response.ok && data.enabled));
  };
  ...
  const runSystemUsageBilling = async (mode: "snapshot" | "invoice") => {
    const label = mode === "snapshot" ? "16日時点の接続数を確定" : "請求書を発行";
    if (mode === "invoice") {
      const typed = window.prompt(`${billingMonth}利用分の本番請求書を発行します。確認のため「${billingMonth}」と入力してください。`);
    ...
    try {
      ...
      setMessage(`${billingMonth}利用分: ${data.processed || 0}件の町内会・自治会を処理しました。${failed ? ` 失敗${failed}件。` : ""}${attention ? ` 決済設定待ち${attention}件。` : ""}`);
      await load();
    } catch (error: any) {
