...
- 世帯主が未登録の場合は、町内会名、郵便番号、住所２、必要に応じて住所３、氏名・カナ氏名で既存名簿を照合し、LINEアカウントを連携します。
- 家族は、世帯主情報（町内会名、住所、世帯主氏名・カナ）で照合し、空いている家族枠へ本人の判断でLINEアカウントを追加できます。世帯主による家族登録や招待URLの発行は行いません。世帯情報と会費レコードは共有し、認証・LINE連携・退会状態は各人で管理します。
- 会費は `resident_rosters.id` 単位の世帯請求です。世帯主と連携済み家族は同じ請求額・入金額・状態を閲覧し、いずれかが支払うと同じ `fee_records` が更新され、世帯全員に納入状況が反映されます。
