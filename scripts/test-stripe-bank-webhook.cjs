...
function fixture(){
  let event,latestInvoice,writes=0,rpcCalls=0;
  ...
  class Stripe{constructor(){this.webhooks={constructEvent:()=>event};this.paymentIntents={retrieve:async()=>({latest_charge:null})};this.invoices={retrieve:async()=>latestInvoice}}}
  ...
  return {tables,notify,setInvoice:value=>{latestInvoice=value},writes:()=>writes,rpcCalls:()=>rpcCalls};
}
(async()=>{
